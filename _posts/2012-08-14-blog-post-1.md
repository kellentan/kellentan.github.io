...
\begin{enumerate}
  ...
  \item $$f(x + y) \leq f(x) + f(y)$$, aka the triangle inequality
  \item $$\forall \alpha \in \mathbb{R},\; f(\alpha x) = |\alpha|f(x)$$
\end{enumerate}

Obviously, in ML we may only care about some of the more canonical norms. The general p-norm is denoted $$\lVert x \rVert_p = (\sum_{i} |x_i|^p)^{\frac{1}{p}}$$. Another handy norm is the infinity norm $$\lVert x \rVert_{\infty} = \max_i |x_i| $$, i.e. simply the element with the largest absolute value. For matrices, we have the analogous Frobenius norm $$\lVert A \rVert_{F} = \sqrt{\sum_{i, j} A_{i, j}^2} = \sqrt{\text{Tr}(A A^T)}$$, which can also be rewritten in terms of the trace operator.
