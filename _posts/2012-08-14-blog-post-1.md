...
Obviously, in ML we may only care about some of the more canonical norms. Another handy norm is the infinity norm $$|| x ||\_{\infty} = \max_i |x_i| $$, i.e. simply the element with the largest absolute value. For matrices, we have the analogous Frobenius norm $$|| A ||\_{F} = \sqrt{\sum_{i, j} A_{i, j}^2} = \sqrt{\text{Tr}(A A^T)}$$, which can also be rewritten in terms of the trace operator.
