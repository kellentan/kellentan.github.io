...
Linear Algebra
======
Recall that the span of a set of m vectors $\{ v^{(1)}, \cdots, v^{(m)} \}$ is simply any point reachable by a linear combination of the vectors, i.e. the set $\{ x\; : \quad x = \sum_{i} c_i v^{(i)}, v^{(i)} \in \{ v^{(1)}, \cdots, v^{(m)} \} \}$. 
