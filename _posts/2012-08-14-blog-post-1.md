...
Linear Algebra
======
Recall that the span of a set of m vectors $v^{(1)}, \cdots, v^{(m)}$ is simply any point reachable by some linear combination of some/all of these m vectors, i.e. the set $$\{x \;:\; x = \sum_{i} c_i v^{(i)}, v^{(i)} \in \{v^{(1)}, \cdots, v^{(m)}\} \}$$. 
