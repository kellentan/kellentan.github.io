...
Obviously, in ML we may only care about some of the more canonical norms. The general p-norm is denoted $$\lVert x \rVert_p = (\sum_{i} |x_i|^p)^{\frac{1}{p}}$$. Another handy norm is the infinity norm $$\lVert x \rVert_{\infty} = \text{max}_{i} |x_i|$$, i.e. simply the element with the largest absolute value. For matrices, we have the analogous Frobenius norm $$\lVert A \rVert_{F} = \sqrt{\sum_{i, j} A_{i, j}^2} = \sqrt{\text{Tr}(A A^T)}$$
